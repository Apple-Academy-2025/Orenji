...
struct DetailCardHistory: View {
    ...
    var body: some View {
        ...
        VStack {
            if prediction.label == "Preparation"{
                ReportHistory(
            ...
            } else if prediction.label == "Bending" {
                ReportHistory(
                    myImage: drawSkeleton(
                        image: convertDataToUIImage(prediction.imageForDisplay) ?? UIImage(),
                ...
                Spacer()
            } else if prediction.label == "FollowThrough" {
                ReportHistory(
                    myImage: drawSkeleton(
                        image: convertDataToUIImage(prediction.imageForDisplay) ?? UIImage(),
    ...
    var body: some View {
        ZStack {
            VStack {
                ...
                .frame(alignment: .top)
                HStack{
                    ForEach(0..<prediction.frames.count, id: \.self) { idx in
                        if idx == selectedTab {
                            Capsule()
        ...
        .fullScreenCover(isPresented: $showFullImage) {
            ZStack(alignment: .topLeading) {
                ...
                if let uiImg =
                    drawSkeleton(
                    image: convertDataToUIImage(prediction.frames[selectedTab].imageForDisplay) ?? UIImage(),
                    handLineColor:colorFunction(angle: Int(prediction.frames[selectedTab].elbowAngle ?? 0),whatAngle:"elbow\(String(describing: prediction.frames[selectedTab].label))" ),
                    legLineColor:colorFunction(angle: Int(prediction.frames[selectedTab].elbowAngle ?? 0),whatAngle:"knee\(String(describing: prediction.frames[selectedTab].label))" )
                    )
                {
                    Image(uiImage: uiImg)
                        .resizable()
